...
def normalize(text: str) -> str:
    ...
    text = re.sub(r'\b(truong|truòng|dai hoc|daihoc|dh|dại học|dai-hoc|dh)\b', ' ', text)
...
def get_benchmark(school: str, major: str, year: int) -> float:
    ...
    if not filtered_df.empty:
        return filtered_df['diem_chuan'].values[0]
    return None

def output_year(school: str, major: str, year: int) -> str:
    """Tạo chuỗi phản hồi cho một năm cụ thể."""
    benchmark = get_benchmark(school, major, year)
    if benchmark is not None:
        return f"Năm {year}: Điểm chuẩn: {benchmark}\n"
